...
abstract class DFormAbstract {
    ...
    public function inactivity_message() {
        return new WP_Error(
            ...
            sprintf( __( '%s plugin is not active', 'propovoice-pro' ), $this->title ),
            [ 'status' => 422 ]
        );
    }
    ...
    public function save( $data ) {
        ...
        /* $response = propovoice()->api->forms()->integrations( StaticStringy::dasherize( $this->slug ) )->post( $settings );

        if ( is_wp_error( $response ) ) {
            return $response;
        }

        update_option( "propovoice_form_integration_{$this->slug}", $form_ids );
