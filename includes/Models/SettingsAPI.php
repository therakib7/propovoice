...
abstract class SettingsAPI {
	 ...
	 * @var string
	 */
	public $plugin_id = 'ncpi_';
	public $plugin_pro_id = 'ncpip';
	...
	public function get_form_fields() {
		return apply_filters(
			'ncpi_settings_api_form_fields_' . $this->option,
	...
	public function process_admin_options() {
		...
		$sanitized_new_settings = apply_filters('ncpi_settings_api_sanitized_fields_' . $this->option, $this->settings, $this);
		do_action('ncpi_admin_settings_before_ddd_' . $this->option, $sanitized_new_settings, Functions::get_option($this->get_option_key()), $this);
	...
	public function generate_text_html($key, $data, $group_name = null, $index = null) {
		...
		$depends       = empty($data['dependency']) ? '' : "data-nd-depends='" . wp_json_encode($data['dependency']) . "'";
		ob_start(); ?>
	...
	public function generate_image_size_html($key, $data) {
        ...
        <tr valign="top" class="<?php echo esc_attr($wrapper_class); ?>" <?php echo wp_kses_post($depends); ?>>
            ...
            <td class="form-input ncpi-image-size-wrap">
                <fieldset>
                    <legend class="screen-reader-text"><span><?php echo wp_kses_post($data['title']); ?></span>
                    </legend>
					<?php foreach ((array) $data['options'] as $option_key => $option_value) : ?>
                        <div class='ncpi-image-size-item'>
							<?php
							if ($option_key == 'crop'): ?>
        ...
        <tr valign="top" class="<?php echo esc_attr($wrapper_class); ?>" <?php echo wp_kses_post($depends); ?>>
            ...
            <td class="form-input">
                <fieldset>
                    ...
                    <div class="ncpi-setting-image-wrap">
                        <input type="hidden" id="<?php echo esc_attr($id); ?>" class="ncpi-setting-image-id"
                        ...
                        <input type="button" class="button button-secondary ncpi-add-image"
                               value="<?php esc_attr_e('Add Image', 'propovoice'); ?>"/>

						<?php if ($value) { ?>
							<input type="button" class="button button-secondary ncpi-remove-image"
                               value="<?php esc_attr_e('Remove Image', 'propovoice'); ?>"/>
						<?php } ?>
        ...
        <tr valign="top" class="<?php echo esc_attr($wrapper_class); ?>" <?php echo wp_kses_post($depends); ?>>
            ...
            <td class="form-input">
                <fieldset>
                    ...
                    <input class="ncpi-color <?php echo esc_attr($data['class']); ?>" type="text"
	...
	public function generate_textarea_html($key, $data) {
		$field_key = $this->get_field_key($key);
		$id        = isset( $data['id'] ) ? esc_attr( $data['id'] ) : $this->get_field_id($key);
		$defaults  = $this->get_placeholder_data(); 
        ...
        <tr valign="top" class="<?php echo esc_attr($wrapper_class); ?>" <?php echo wp_kses_post($depends); ?>>
            ...
            <td class="form-input">
                <fieldset>
                    ...
                    <textarea rows="9" cols="20" class="input-text wide-input <?php echo esc_attr($data['class']); ?>"
            ...
            <td class="form-input">
                <fieldset>
					...
					<?php foreach ((array) $data['options'] as $option_key => $option_value) : 
						...
						$is_pro = false;  
						/* if ( isset($data['is_pro']) && ! $this->is_pro() ) {
							if ( $field_key == 'ncpi_general_settings[post_type]' ) {
								$premium_type = ['product', 'download'];
								if ( in_array( $option_key, $premium_type) ) {
									$is_pro = true; 
								}
							} else if ( $field_key == 'ncpi_general_settings[taxonomy]' ) {
								$premium_type = ['product_cat', 'product_tag', 'download_category', 'download_tag'];
								if ( in_array( $option_key, $premium_type) ) {
									$is_pro = true; 
								}
							}
						...
						if ( !$is_pro ) { 
						?>
                        <label for="<?php echo esc_attr($id . '-' . $option_key); ?>" class="<?php $is_pro ? $this->pro_parent_wrap() : ''; ?>">
                            <input class="<?php echo esc_attr($data['class']); ?>" type="checkbox"
								name="<?php echo esc_attr($field_key); ?>[]"
								id="<?php echo esc_attr($id . '-' . $option_key); ?>"
								value="<?php echo esc_attr($option_key); ?>"
								<?php if ( $is_pro ) echo 'disabled'; ?>
								<?php if ( !$is_pro ) checked(in_array($option_key, $values)); ?> />
							<?php echo esc_html( $option_value ); ?>
							<?php $is_pro ? $this->pro_label() : ''; ?>
							<?php $is_pro ? $this->pro_wrap() : ''; ?>
                        </label><br/>
					<?php 
					} 
					endforeach; ?>
			...
			$value
					...
					<?php if ($data['select_buttons']) : ?>
                        <br/><a class="select_all button"
                                href="#"><?php esc_html_e('Select all', 'propovoice'); ?></a> <a
                                class="select_none button"
                                href="#"><?php esc_html_e('Select none', 'propovoice'); ?></a>
					<?php endif; ?>
                </fieldset>
            </td>
        </tr>
	...
	public function is_pro() {
		return function_exists( $this->plugin_pro_id );
	}

	public function pro_label() {
		if ( ! $this->is_pro() ) {
			printf(
				'<span title="%s" style="color: red;">%s</span>',
				esc_html__( 'This is a premium field', 'propovoice' ),
				esc_html__( '[Pro]', 'propovoice' )
			);
		}
	}

	public function pro_parent_wrap() {
		if ( ! $this->is_pro() ) {
			echo 'ncpi-pro-parent_wrap';
		}
	}

	public function pro_wrap() {
		if ( ! $this->is_pro() ) {
			echo '<div class="ncpi-pro-wrap"></div>';
		}
	}
	...
	public function generate_title_html($key, $data) {
        ...
        <h3 class="ncpi-settings-sub-title <?php echo esc_attr($data['class']); ?>"
            id="<?php echo esc_attr($id); ?>">
			<?php echo wp_kses_post($data['title']); ?>
			<?php isset($data['is_pro']) ? $this->pro_label() : ''; ?>
		</h3>

		<?php isset($data['is_pro']) ? $this->pro_wrap() : ''; ?>
	...
	private function get_placeholder_data() {
		return [
			...
			'placeholder'       => '',
			'blank'             => true,
			'blank_text'        => esc_html__('Select one', 'propovoice'),
			'blank_value'       => '',
			'type'              => 'text',
			'desc_tip'          => false,
			'description'       => '',
