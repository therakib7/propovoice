...
const General = () => {
  ...
  const updateSingleAction = (type, action_id, action_slug, data) => {
    ...
    setUserPreferences((prev) => {
      return {
        ...prev,
        [action_slug]: { ...prev[action_slug], [type]: newSingleIsEnabled },
      };
    });
  ...
  const changeAllPreferenceState = (type, value) => {
    Object.keys(userPreferences).map((slug) => {
      setUserPreferences((prev) => {
  ...
  const getUserPreference = (args) => {
    api
      .get(`notifications/users/${ndpv.profile.id}/preferences`, args, "pro")
      .then((resp) => {
        const preferences = resp.data;
        preferences.map((preference) => {
          const { slug, action_id, label, notification_type, is_enabled } =
            preference;
          ...
          setUserPreferences((prev) => ({
            ...
            [slug]: {
              ...
              action_id: action_id,
              label: label,
              [notification_type]: parseInt(is_enabled),
            },
          }));
        });
      });
  ...
  const submitUserPreferences = (data) => {
    api
      .add(`notifications/users/${ndpv.profile.id}/preferences`, data, "pro")
      .then((resp) => {
  ...
  return (
    <>
      <h4 className="pv-title-medium pv-mt-15 pv-mb-15">
      ...
      <table className="pv-table" style={{ marginTop: "50px" }}>
        ...
        <tbody>
          {Object.keys(userPreferences).map((slug, index) => (
            <tr key={index}>
                ...
                <Checkbox
                  ...
                  changeHandler={() =>
                    handleNotificationChange(
                      "mail",
                      userPreferences[slug].action_id,
                      slug,
                    )
                  }
                />
