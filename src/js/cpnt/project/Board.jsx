...
const onDragEnd = (update, result, columns, setColumns) => {
	...
	if (source.droppableId !== destination.droppableId) {
		...
		let project_id = parseInt(columns[source.droppableId].items[source.index].id);
		let project_status_id = parseInt(destColumn.id);
		let finalArray = destItems.map(function (obj) {
			return parseInt(obj.id);
		});
		update('projects', project_id, { status_id: project_status_id, reorder: finalArray }); //update come from api hoc
		...
		//update project in same status
		let finalArray = copiedItems.map(function (obj) {
			return parseInt(obj.id);
		});
		let project_id = parseInt(columns[source.droppableId].items[source.index].id);
		update('projects', project_id, { reorder: finalArray });
	}
};
