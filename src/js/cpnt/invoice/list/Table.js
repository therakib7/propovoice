    ...
    let rows = props.tableData.map((row, index) => {
        ...
        return (
            <tr key={index}>
                ...
                <td onClick={() => { handleClick(row, '', isClient, client_url); }} className='pv-cursor-pointer'><span className='pv-list-title'>{nNum}</span></td>
                {/*<td>{row.project.name}</td>*/}
                {!isClient && !props.client_id && <td onClick={() => { handleClick(row, '', isClient, client_url); }} className='pv-cursor-pointer'>
                    {(row.to.type == 'person') ? row.to.first_name : row.to.org_name}
                </td>}
                <td onClick={() => { handleClick(row, '', isClient, client_url); }} className='pv-cursor-pointer'>{currency(row.total, row.invoice.currency, row.invoice.lang)}</td>
                {/* {(props.path == 'invoice') &&
                    <>
                        <td>{row.paid}</td>
                        <td>{row.due}</td>
                ...
                <td onClick={() => { handleClick(row, '', isClient, client_url); }} className='pv-cursor-pointer'><Moment format={ndpv.date_format}>{row.date}</Moment></td>
